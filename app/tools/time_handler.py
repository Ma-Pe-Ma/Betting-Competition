...
def init_time_handler(app : Flask) -> None:
    if app.debug:
        ...
        app_start_time = datetime.utcnow().replace(tzinfo=tz.gettz('UTC'))

        global get_now_time_object
        get_now_time_object = lambda : debug_start_time + (datetime.utcnow().replace(tzinfo=tz.gettz('UTC')) - app_start_time)
