...
match_evaluation_query_string = text(
                            "WITH match_prize AS("
                                "SELECT match.id, m_outcome.outcome AS match_outcome, b_outcome.outcome AS bet_outcome, COALESCE(m_outcome.outcome = b_outcome.outcome, 0) AS success, "
                                    ...
                                    "END AS multiplier, "
                                    "CASE m_outcome.outcome = b_outcome.outcome "
                                        "WHEN 1 THEN CASE WHEN match.goal1 = b_outcome.goal1 AND match.goal2 = b_outcome.goal2 "
                                            "THEN :bullseye "
                                            "ELSE CASE WHEN (match.goal1 - match.goal2) = (b_outcome.goal1 - b_outcome.goal2) "
                                                "THEN :difference "
                                                "ELSE 0 "
                                                "END "
                                            "END "
