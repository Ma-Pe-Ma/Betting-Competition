...
tournament_dict = '''SELECT t_bet.id, t_bet.username, team, result, success, COALESCE(t_bet.bet, 0) AS bet, local_name, multiplier, 
...
group_dict = '''SELECT ts2.*, COALESCE(ts2.multiplier * ts2.bet, 0) AS prize, (ts2.multiplier - 1) * ts2.bet AS credit_diff 
            FROM (
                ...
                FROM (SELECT team.name, team.group_id, team.position, (team.name = tb.team) AS hit, tr1.translation AS name_tr, COALESCE(group_bet.bet, 0) AS bet, 
                    COALESCE(tb.team, team.name) AS bet_name, COALESCE(tr2.translation, tr1.translation) AS bet_name_tr, 
...
def get_daily_points_by_current_time_query(users : str) -> str:
    simple_entry_query = '''SELECT date_values.*
                    FROM (SELECT date.datetime, date(date.datetime) AS date, NULL AS id
                            FROM (SELECT DATE('{st}', '{days}' || ' days') AS datetime) AS date
                            {filter}) AS date_values
                    UNION ALL '''

    match_query = '''SELECT * FROM 
                        (SELECT match.datetime AS datetime, date(match.datetime) AS date, match.id AS id
                        FROM match
    ...
    complete_query += 'SELECT 1, 2, 3 WHERE 0 = 1'
    ...
    complete_query = '''
    SELECT days.username, date(days.datetime) AS date, 
...
def get_group_and_tournament_bet_amount(username : str) -> int:
    ...
    result_object = result.fetchone()
    return result_object._asdict()['total_bet'] if result_object is not None else 0
...
def get_group_bet_dict_for_user(username : str, language = None):
    ...
    for team_row in team_rows:
        ...
        for gr in groups:
            if gr['id'] == team_row.group_id:
                current_group = gr
                break

        if current_group is None:
            current_group = {'id': team_row.group_id, 'hit_number' : team_row.hit_number, 'bet' : team_row.bet, 'prize' : team_row.prize, 'credit_diff' : team_row.credit_diff, 'teams' : [], 'multiplier' : team_row.multiplier}
        ...
        del team_dict['credit_diff']
        del team_dict['group_id']
        del team_dict['username']        

        current_group['teams'].append(team_dict)
